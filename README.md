...
- GitHub Actions에서는 06:00부터 기사를 수집하고 Copilot CLI로 20~30자의 실무
  참고 메시지를 생성한 뒤, 최종 Telegram 메시지를 JSON으로 준비합니다.
- 준비 JSON에는 최종 메시지와 생성 건수만 저장하며 기사 본문은 저장하지 않습니다.
- 06:30에는 수집을 다시 하지 않고 준비된 JSON만 Telegram으로 전송합니다.
- 과천시·대전시의 당일 날씨는 Open-Meteo Forecast API에서 수집해 메시지 상단에
  표시합니다.
- Open-Meteo는 KMA Seamless 모델을 우선 요청하며, KMA 값이 비어 있으면 자동 모델로
  재조회해 당일 예보 발행을 보장합니다.
