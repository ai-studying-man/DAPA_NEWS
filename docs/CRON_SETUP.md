...
- 한국 시간대 cron: `30 6 * * *`
- UTC 기준 cron: `30 21 * * *`
- GitHub Actions는 06:00부터 뉴스·보도자료·실무 참고 메시지를 준비합니다.
...
최초 실행은 `data/initial-press-releases.json`의 2026년 8월 5일 검증값으로
부트스트랩하고, 이후 실행은 공식 게시판에서 실시간으로 더 최신인 자료를 갱신합니다.
공식 게시판이 일시적으로 응답하지 않으면 직전 최신값을 사용합니다. 특정 기관의
실시간 자료와 저장값이 모두 없어도 전체 뉴스 발송은 계속하며 잘못된 `없음` 문구는
표시하지 않습니다.
