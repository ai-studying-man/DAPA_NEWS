...
정책브리핑 방위사업청 RSS는 필수 수집원이다. 이 피드의 수집 성공 여부와 기사 수를 매 실행마다 기록하며, 실패를 단순한 `수집 기사 없음`으로 처리하지 않는다.
...
- 정책브리핑 방위사업청 RSS와 Google News의 `"방위사업청" OR "방사청"` 전용 쿼리를 독립적으로 실행해 상호 누락을 보완한다.
...
- 현 정부 뉴스는 평면형 `직위자/정부 + 방산/국방` 검색 쿼리로 수집하고 제목·RSS 설명·출처를 함께 분류한다.
